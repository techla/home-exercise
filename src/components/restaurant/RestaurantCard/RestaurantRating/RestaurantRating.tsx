...
const RestaurantRating: React.FC<RestaurantRatingProps> = ({
  ...
  return (
    <div className={styles.rating}>
      {ratingValue && (
        <strong className={styles.score} itemProp="ratingValue" aria-label={`Rating: ${ratingValue} out of 5`}>
          {ratingValue}
        </strong>
      )}
      {reviewCount && (
        <div itemProp="reviewCount" className={styles.reviewCount} aria-label={`${reviewCount} reviews`}>
          ...
          <span className={styles.countNumber}>{(reviewCount).toLocaleString('fr-FR')}</span>
        </div>
      )}
    </div>
  );
